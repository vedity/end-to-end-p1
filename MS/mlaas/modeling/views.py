...
class LiftChartClass(APIView):
    
        def get(self, request, format=None):
                """
                This function is used to get Lift Chart values of particular experiment.
                ...
                try:
                        ...
                        lift_values = ModelStatObject.show_lift_chart(experiment_id)#call show lift chart method which will return train size,test size and test score
                        logging.info("modeling : ModelStatisticsClass : GET Method : execution stop : status_code :200")
                        if isinstance(lift_values,str): #check the instance of dataset_df
                                status_code,error_msg=json_obj.get_Status_code(lift_values) # extract the status_code and error_msg from project_df
                                logging.info("modeling : ModelStatisticsClass : GET Method : execution : status_code :"+ status_code)
                                return Response({"status_code":status_code,"error_msg":error_msg,"response":"false"})
                        else:
                                logging.info("modeling : ModelStatisticsClass : GET Method : execution : status_code : 200")
                                return Response({"status_code":"200","error_msg":"successfull retrival","response":lift_values})                   
